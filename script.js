...
// =====================================================================
// === COMPACT SUBTASK TREE-TABLE (adds density; unlimited nesting)  ===
// =====================================================================
const ensureSubtaskTableStyles = () => {
  ...
  .inline-add{background:var(--color-bg-input)}
  .inline-add input,.inline-add select{margin:0}
...
const renderSubtasksAsTreeTable = (rootTask) => {
  ...
  // build visible rows (skip Closed subtasks to match current UI)
  const rows = [];
  ...
  const addRow = ({ task, parentId, level }) => {
    ...
    statusTd.innerHTML = `<select class="subtask-status" data-task-id="${task.id}">
      ${KANBAN_STATUSES.map(s => optionHTML(s, s === task.status)).join('')}
    </select>`;
