...
if(activeLink) {
        ...
        if (isTaskView && typeof renderKanbanBoard === 'function') { try { renderKanbanBoard(); } catch (e) { console.error('renderKanbanBoard failed:', e); } }
        const _searchEl = document.querySelector('.search-container') || get('search-container');
        if(_searchEl) _searchEl.style.display = isTaskView ? 'flex' : 'none';
    };
...
[assigneeFilter, categoryFilter, sortBySelect, closedFilterSelect].forEach(el => el.addEventListener('change', renderKanbanBoard));
