...
const queryAll = (selector) => document.querySelectorAll(selector);
   ...
   addSubTaskFormContainer = get('addSubTaskFormContainer'), mainLogControls = get('mainLogControls'), logSummarySection = get('logSummarySection'), logSummaryContainer = get('logSummaryContainer'), reportsLink = get('reportsLink'), settingsBtn = get('settingsBtn'), settingsModal = get('settingsModal'), peopleList = get('peopleList'), categoryList = get('categoryList'), settingsPersonForm = get('settingsPersonForm'), settingsCategoryForm = get('settingsCategoryForm'), settingsPersonNameInput = get('settingsPersonName'), settingsCategoryNameInput = get('settingsCategoryName'), passwordList = get('passwordList'), settingsPasswordForm = get('settingsPasswordForm'), settingsPasswordServiceInput = get('settingsPasswordService'), settingsPasswordUsernameInput = get('settingsPasswordUsername'), settingsPasswordValueInput = get('settingsPasswordValue'), openPasswordModalBtn = get('openPasswordModalBtn'), passwordModal = get('passwordModal'), passwordModalTitle = get('passwordModalTitle'), passwordIdInput = get('passwordId'), settingsPasswordLinkInput = get('settingsPasswordLink'), linkModal = get('linkModal'), linkForm = get('linkForm'), linkNameInput = get('linkName'), linkUrlInput
    = get('linkUrl'), existingLinksList = get('existingLinksList'), websiteList = get('websiteList'), openWebsiteModalBtn = get('openWebsiteModalBtn'), websiteModal = get('websiteModal'), globalSearchInput 
...
logAction(task, `Task status changed to Closed.`);
    ...
    const getAllTasksAndSubtasks = () => {
        const flatList = [];
        function traverse(tasks, parentName = null, parentId = null, parentCategory = null) { // Added parentCategory
            for (const task of tasks) {
                // A subtask inherits its parent's category if it doesn't have its own.
                const effectiveCategory = task.category || parentCategory;
                // Create a clone to avoid modifying the original object, now with the correct category
                const taskClone = { ...task, parentTaskName: parentName, parentId: parentId, category: effectiveCategory };
                flatList.push(taskClone); [cite_start]// [cite: 57]
                if (task.subtasks && task.subtasks.length > 0) {
                    // Pass current task's name, id, and effective category down to its children
                    traverse(task.subtasks, task.name, task.id, effectiveCategory);
                }
            }
        }
        traverse(tasks.filter(t => !t.isArchived));
        return flatList; [cite_start]// [cite: 59]
    };
...
const weekTasksContainer = weekColumn.querySelector('.tasks-container');
    ...
    const createTaskCard = (task, isDraggable = true) => {
        ...
        const today = new Date(); [cite_start]// [cite: 86]
        today.setHours(0,0,0,0);
        const isLate = new Date(task.dueDate) < today && !['Closed'].includes(task.status);
        card.className = `task-card ${isLate ? 'late' : ''}`; [cite_start]// [cite: 87]
        card.dataset.id = task.id; [cite_start]// [cite: 87]
        card.draggable = isDraggable;

        // This logic is already correct for getting the root parent's category for the color band and tag.
        const rootTask = findTaskById(task.parentId || task.id) || task;
        const categoryColor = categories[rootTask.category] || 'transparent'; [cite_start]// [cite: 88]
        card.style.setProperty('--card-category-color', categoryColor);
        
        const allAssignees = Array.from(getAllAssignees(task));
        const assigneeStackHTML = allAssignees.map(p => `<div class="card-assignee-icon" style="background-color: ${people[p] || '#ccc'}" title="${p}">${getInitials(p)}</div>`).join(''); [cite_start]// [cite: 89]
        const urgentIconHTML = task.isUrgent ? '<i class="fas fa-exclamation-circle urgent-icon" title="Urgent"></i>' : ''; [cite_start]// [cite: 90]
        const quickCloseBtnHTML = task.status !== 'Closed' ? `<button class="task-card-quick-close" data-task-id="${task.id}" title="Mark as Closed"><i class="fas fa-check-circle"></i></button>` : ''; [cite_start]// [cite: 91]
        
        // --- NEW: Logic for sub-task badge and title layout ---
        const subtaskBadgeHTML = task.parentTaskName ? '<div class="subtask-badge">Sub-task</div>' : '';
        const titleHTML = `<span>${task.name}</span>`;
        const parentIndicatorHTML = task.parentTaskName ? `<small class="parent-task-indicator">of ${task.parentTaskName}</small>` : '';

        let displayProgress = 0;
        [cite_start]if (task.status === 'Closed') { // [cite: 92]
            displayProgress = 100;
        [cite_start]} else if (typeof task.progress === 'number' && task.progress >= 0) { // [cite: 93]
            displayProgress = task.progress;
        [cite_start]} else if (task.status === 'In Progress') { // [cite: 94]
            displayProgress = 50;
        }
        ...
        // --- MODIFIED: The card's inner HTML structure ---
        card.innerHTML = `
            ${quickCloseBtnHTML}
            ${urgentIconHTML}
            ${subtaskBadgeHTML}
        ...
        [cite_start]card.addEventListener('click', (e) => { // [cite: 101]
            if (e.target.closest('.task-card-quick-close')) return;
            const rootParentId = task.parentId || task.id;
            openTaskModal(rootParentId)
        });
        if (isDraggable) {
            card.addEventListener('dragstart', handleDragStart); [cite_start]// [cite: 102]
            card.addEventListener('dragend', handleDragEnd); [cite_start]// [cite: 102]
        }

        const quickCloseBtn = card.querySelector('.task-card-quick-close'); [cite_start]// [cite: 103]
        [cite_start]if (quickCloseBtn) { // [cite: 104]
            quickCloseBtn.addEventListener('click', (e) => {
                ...
                if (taskToClose && taskToClose.status !== 'Closed') {
                    ...
                    [cite_start]taskToClose.closedDate = new Date().toISOString(); // [cite: 105]
                    logAction(taskToClose, `Task status changed to Closed.`); [cite_start]// [cite: 105]
                    saveState();
                }
            });
        }
...
const updateMainTaskDueDate = (task) => {
        if (task.isUrgent) {
            return;
// Do not auto-update urgent tasks
        }
...
if (allSubtasks.length === 0) {
            return;
// No open subtasks to base the date on
        }
