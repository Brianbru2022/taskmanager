...
div.querySelectorAll('.btn-chaser').forEach(btn => btn.addEventListener('click', (e) => {logAction(taskObject, e.target.dataset.log, taskObject.assignee); openTaskModal(currentEditingTask.id);}));
    ...
    // =====================================================================
    // COMPACT SUBTASK TREE-TABLE RENDERER + INLINE ADD ROW
    // =====================================================================
    const renderSubtasksAsTreeTable = (task) => {
        const container = subtasksListContainer;
        container.innerHTML = '';

        const addRootBtn = document.createElement('button');
        addRootBtn.className = 'btn btn-primary add-root-subtask';
        addRootBtn.innerHTML = `<i class="fas fa-plus"></i> Add Sub-Task`;
        addRootBtn.addEventListener('click', () => showInlineAddRow(task.id, 0, null));
        ...
        table.className = 'subtask-tree-table';
        table.innerHTML = `
          <thead>
            <tr>
              <th>Task</th>
        ...
        const addRow = (item, level) => {
            ...
            const isOverdue = item.dueDate ? (new Date(item.dueDate) < today && item.status !== 'Closed') : false;
            if (isOverdue) tr.classList.add('row-overdue');

            const hasChildren = (item.subtasks && item.subtasks.length > 0);
            const isExpanded = expandedSubtasks.has(item.id);

            const caretHTML = hasChildren
              ? `<span class="tree-caret" aria-label="${isExpanded ? 'Collapse' : 'Expand'}" title="${isExpanded ? 'Collapse' : 'Expand'}"><i class="fas fa-chevron-down"></i></span>`
              : `<span class="tree-caret placeholder"></span>`;

            const assigneeIcon = `<div class="card-assignee-icon compact-assignee" style="background-color:${people[item.assignee] || '#ccc'}" title="${item.assignee || ''}">${getInitials(item.assignee)}</div>`;
            const linksCount = (item.links || []).length || '';

            tr.innerHTML = `
              <td class="tree-cell" style="--indent:${level * 16}px">
                ${caretHTML}
                <span class="tree-title">${item.name}</span>
              </td>
              <td class="assignee-cell">
                ${assigneeIcon}<span class="assignee-name">${item.assignee || '—'}</span>
              </td>
              <td>${formatDateForDisplay(item.dueDate)}</td>
              <td>
                <select class="row-status" data-id="${item.id}">
                  ${KANBAN_STATUSES.map(s => `<option value="${s}" ${item.status === s ? 'selected' : ''}>${s}</option>`).join('')}
                </select>
              </td>
              <td>
                <button class="link-btn" data-id="${item.id}" title="Manage Links">
                  <i class="fas fa-link"></i> ${linksCount}
                </button>
              </td>
              <td class="row-actions">
                <button class="icon-btn add-child" data-id="${item.id}" title="Add sub-task"><i class="fas fa-plus"></i></button>
                <button class="icon-btn quick-close" data-id="${item.id}" title="Mark closed"><i class="fas fa-check-circle"></i></button>
              </td>
            `;

            const caret = tr.querySelector('.tree-caret');
            if (caret && !caret.classList.contains('placeholder')) {
                if (!isExpanded) tr.classList.add('collapsed');
                caret.addEventListener('click', (e) => {
                    e.stopPropagation();
                    if (isExpanded) expandedSubtasks.delete(item.id);
                    else expandedSubtasks.add(item.id);
                    renderSubtasksAsTreeTable(currentEditingTask);
                });
            }

            tr.querySelector('.row-status').addEventListener('change', (e) => {
                const st = findTaskById(e.target.dataset.id);
                if (!st) return;
                st.status = e.target.value;
                if (st.status === 'Closed') {
                    st.closedDate = new Date().toISOString();
                    logAction(currentEditingTask, `Sub-task "${st.name}" marked as completed.`, st.assignee);
                    updateMainTaskDueDate(currentEditingTask);
                }
                saveState();
                renderSubtasksAsTreeTable(currentEditingTask);
                renderLogSummary(currentEditingTask);
            });

            tr.querySelector('.quick-close').addEventListener('click', (e) => {
                const st = findTaskById(e.currentTarget.dataset.id);
            ...
            tr.querySelector('.link-btn').addEventListener('click', (e) => {
                openLinkModal(e.currentTarget.dataset.id);
            });

            tr.querySelector('.add-child').addEventListener('click', () => {
                showInlineAddRow(item.id, level + 1, tr);
            });

            tbody.appendChild(tr);

            if (isExpanded && hasChildren) {
                item.subtasks.forEach(child => addRow(child, level + 1));
            }
        };

        (task.subtasks || []).forEach(st => addRow(st, 0));
    };

    const showInlineAddRow = (parentId, level, insertAfterRow) => {
        const existing = document.getElementById('inline-add-row');
        if (existing) existing.remove();

        const tbody = subtasksListContainer.querySelector('tbody') || subtasksListContainer.querySelector('.subtask-tree-table tbody');
        const tr = document.createElement('tr');
        tr.id = 'inline-add-row';
        tr.innerHTML = `
          <td class="tree-cell" style="--indent:${level * 16}px">
            <span class="tree-caret placeholder"></span>
            <input class="ia-title" type="text" placeholder="Title" />
          </td>
          <td>
            <div class="input-with-button" style="min-width:220px;">
              <select class="ia-assignee"><option value="" disabled selected>Select...</option></select>
              <button type="button" class="add-icon-btn ia-add-person" title="Add person"><i class="fas fa-user-plus"></i></button>
            </div>
          </td>
          <td><input class="ia-due" type="date"/></td>
          <td><span class="muted">Open</span></td>
          <td></td>
          <td class="row-actions">
            <button class="icon-btn ia-save" title="Save"><i class="fas fa-check"></i></button>
            <button class="icon-btn ia-cancel" title="Cancel"><i class="fas fa-times"></i></button>
          </td>
        `;

        if (insertAfterRow && insertAfterRow.parentNode) {
            insertAfterRow.parentNode.insertBefore(tr, insertAfterRow.nextSibling);
        } else if (tbody) {
            tbody.appendChild(tr);
        } else {
            subtasksListContainer.appendChild(tr);
        }

        const sel = tr.querySelector('.ia-assignee');
        sel.innerHTML += Object.keys(people).sort().map(p => `<option value="${p}">${p}</option>`).join('');
        tr.querySelector('.ia-add-person').addEventListener('click', () => {
            activeAssigneeSelect = sel;
            openModal(personModal);
        });
        tr.querySelector('.ia-cancel').addEventListener('click', () => tr.remove());
        tr.querySelector('.ia-save').addEventListener('click', () => {
            const name = tr.querySelector('.ia-title').value.trim();
            const assignee = sel.value;
            const dueDate = tr.querySelector('.ia-due').value;
            if (!name || !assignee || !dueDate) { alert('Please provide a title, assignee, and due date.'); return; }

            const parent = (parentId === currentEditingTask.id) ? currentEditingTask : findTaskById(parentId);
            if (!parent.subtasks) parent.subtasks = [];
            parent.subtasks.push({
                id: getNextId('SUB'), name, description:'', assignee, dueDate,
                status:'Open', subtasks:[], log:[], closedDate:null, progress:null, links:[]
            });
            logAction(parent, `Sub-task "${name}" was added.`, assignee);
            updateMainTaskDueDate(currentEditingTask);
            saveState();
            openTaskModal(currentEditingTask.id);
        });
